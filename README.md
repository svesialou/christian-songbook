...
- Транспонирование выполняется на уровне конкретной открытой песни; нижняя BPM-панель включается в настройках и поддерживает ручной ввод, шаги `-/+`, tap tempo, повтор припева по заголовкам вроде `[Припев x2]`, слоговую оценку длинных строк и явные подсказки времени вроде `2 такта` или `8 долей` для проигрышей.
