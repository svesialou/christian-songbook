...
- Live-режим открывается верхней кнопкой `Live` только после входа, выбирает песни из всего каталога, поддерживает отдельные live-сборники, сброс текущей очереди и отправку через Web Share/clipboard.
- Live-сборники привязаны к аккаунту через backend/MySQL и доступны после входа на другом устройстве.
- Поиск в основном списке песен остаётся закреплённым сверху при прокрутке каталога, ищет по заголовку и тексту песни, подсвечивает найденные подряд вхождения и игнорирует регистр и знаки препинания между словами.
- В длинных списках есть кнопка перехода наверх с возвратом на позицию, откуда пользователь ушёл.
- Внутри песни есть режим презентации: клик по превью слайда открывает его на весь экран для второго экрана или расшаренного окна.
- Внутри песни есть печать текущего текста с аккордами рядом с отправкой через Web Share/clipboard.
- При загрузке сборников приложение пробует обновить backend snapshot и сохранить свежие песни локально для offline; зелёный статус означает подтверждённый свежий snapshot из backend.
- Пока online-синхронизация каталога выполняется, приложение показывает фоновый loader вокруг индикатора без блокировки скролла; offline открывает сохранённый каталог сразу, а пустой каталог подсказывает pull-to-refresh.
- Транспонирование выполняется на уровне конкретной открытой песни; нижняя BPM-панель включается в настройках и поддерживает ручной ввод, шаги `-/+`, tap tempo, повтор пред припева/припева после куплетов/бриджа и по заголовкам вроде `[Припев x2]`, слоговую оценку длинных строк и явные подсказки времени вроде `2 такта` или `8 долей` для проигрышей.
- Внутри песни можно быстро уменьшать или увеличивать размер шрифта без открытия настроек; ширина области текста и аккордов адаптируется к выбранному размеру.
- Пользователь может предложить правку опубликованной песни без admin-доступа; правка попадает в pending заявки и применяется только после апрува в `/admin`.
...
- В панели музыканта есть локальные view presets `Lead`, `Singer`, `Chords` как foundation для будущих ролей команды; live-сборники уже сохраняются per-user через backend.
...
- Сейчас реализован admin slice: просмотр и редактирование pending заявок, approve в текущий MySQL-каталог, прямое добавление песни, редактирование опубликованных песен, soft delete/скрытие песен, единого `leadSheet` и playback-параметров. Полноценные пользователи, роли и audit log остаются отдельными задачами.
- В admin `leadSheet` заголовки куплетов распознаются в обоих порядках: `[Куплет 1]` и `[1 куплет]`.
- Admin API не даёт добавить или переименовать опубликованную песню в уже существующее название внутри текущего каталога.
- Список песен в админке показывает каталог порциями при прокрутке, а поиск по названию/автору/категории/номеру дополнительно запрашивает MySQL backend.
- Заявки могут быть новыми песнями или правками существующих песен. Для правок admin UI показывает line diff между исходным `leadSheet` и предложенным текстом, а approve обновляет исходную песню.
...
- Если аккорды размечены только в первом куплете, просмотр и редакторы могут подтянуть их в остальные куплеты без перетирания уже указанных аккордов.
